...
public interface AddressRepository extends JpaRepository<Address, Integer> {
    ...
    @Query("SELECT a FROM Address a WHERE a.user.userId = :customerId AND a.addressStatus <> -1")
    List<Address> findByCustomerID(@Param("customerId") int customerId);

    @Query("SELECT a FROM Address a WHERE a.user.userId = :customerId AND a.addressStatus = 2")
